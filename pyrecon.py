...
def Usage():
    ...
    print '--connect-timeout - sets the WGet --connect-timeout value in seconds, default 90'
...
'''
def WGet(target, logdir, connectTimeout=90):
    ...
    subproc = subprocess.Popen('wget --tries=5' + ' --connect-timeout ' + str(connectTimeout) + ' -S -O ' + filename + ' ' + target, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    ...
    subproc = subprocess.Popen('wget --tries=5' + ' --connect-timeout ' + str(connectTimeout) + ' -S -O ' + filename + ' ' + target + ':443', shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    ...
    subproc = subprocess.Popen('wget --tries=5' + ' --connect-timeout ' + str(connectTimeout) + ' -S -O ' + filename + ' ' + target + ':8080', shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
if __name__ == '__main__':
            ...
            for current_IP in IP:
                ...
                if not os.path.exists(logdir):
                    ...
                    if (AP.supresswget == False): 
                        WGet(current_IP, logdir, AP.connectTimeout)
        ...
        if ((AP.mode == 'manual') or (AP.mode == 'fail2ban')):
            ...
            if not os.path.exists(logdir):
                ...
                if (AP.supresswget == False):
                    WGet(AP.target, logdir, AP.connectTimeout)
                if (AP.supressemail == False):
